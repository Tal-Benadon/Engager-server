...
async function login(email, password) {
  ...
  //  הפקת טוקן בכניסה
  const token = jwt.sign({ phone: user.phone }, process.env.SECRET, {
    expiresIn: "7d",
  });
  return { token, user };
}
...
const checkToken = async (token) => {
  try {
    ...
    const campaign = await campaignController.read(
      { _id: approval.userId },
      { _id: approval.campaignId }
    );
    if (!campaign) throw { msg: "campaign not found" };
...
const sendToAddLead = async (token, data) => {
  ...
  return (
    "the lede create" +
    (await leadService.addLeadToCamp(campaignId, userId, data.data, token))
  );
};
