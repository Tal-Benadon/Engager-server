...
async function sendFeedback(userId, data) {
  ...
  if (!userExist)
    throw { code: 500, msg: "User does not exist, cannot create feedback." };
...
async function getFeedbackById(userId) {
  ...
  if (!userExist)
    throw { code: 500, msg: "User not found, cannot get feedback." };

  //if there is no feedbacks for this user
  if (feedbacks.length === 0)
    throw { code: 404, msg: "No feedbacks found for this user." };
