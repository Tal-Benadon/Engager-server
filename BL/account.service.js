...
async function updateOneUserPassword(phone, data) {
  ...
  if (password?.length < 8)
    throw {
      code: 408,
      msg: "The password does not contain at least 8 characters",
    };
  if (!passwordRegex.test(password))
    throw {
      code: 408,
      msg: "The password does not contain at least 1 leter and 1 number",
    };
...
async function createNewUser(body) {
  ...
  if (!email.includes("@") || !email.includes("."))
    throw { code: 408, msg: "Email is not proper" };
  ...
  scheduleService.convertToDateAndExec(expiredDate, () =>
    endOfTrialPeriod(phone)
  );
...
async function confirmNewUser(token) {
  try {
    ...
    if (userToConfirm.length < 1)
      throw { code: 401, msg: "User does not exist" };
    if (userToConfirm[0].isActive == true)
      return {
        successStatus: "AlreadyActive",
        msg: "User is already active",
        user: userToConfirm[0],
      };
    ...
    console.error(err);
    return {
      successStatus: "ActivationFailed",
